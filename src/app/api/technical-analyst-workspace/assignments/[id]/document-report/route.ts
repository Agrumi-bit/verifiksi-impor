...
/**
 * Technical Analyst mirror of `verifikator-workspace/assignments/[id]/document-report/route.ts`.
 * The "technical" Assignment row has no document decisions of its own — those live on the sibling
 * "dokumen" (verifikator) Assignment row for the same applicationId, same split as
 * `technical-analyst-workspace/assignments/[id]/route.ts`'s `loadSiblingSummaries`. Only exposed
...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!technicalAssignment || technicalAssignment.technicalReviewerId !== technicalAnalystId) {
    return NextResponse.json({ error: "Penugasan tidak ditemukan" }, { status: 404 });
  }

  const dokumenSiblings = await db.assignment.findMany({
    where: { applicationId: technicalAssignment.applicationId, verifikatorId: { not: null } },
    include: { application: true, verifikator: true, technicalReviewer: true },
  });
  // A reschedule can leave multiple "dokumen" sibling rows — the completed one
  // (if any) is the real report, never an earlier still-in-progress duplicate.
  const assignment = dokumenSiblings.find((a) => a.status === "COMPLETED") ?? null;
  if (!assignment) {
