...
import { surveyReportDraftSchema } from "@/modules/surveyor-workspace/schema";
...
export async function PATCH(
  ...
  const parsed = surveyReportDraftSchema.safeParse(body);
  if (!parsed.success) {
    return NextResponse.json(
  ...
  const updated = await db.locationVisit.update({
    ...
    data: {
      ...
      photos: values.evidence,
      findings: values.findings,
      fieldObservationNotes: values.notes || null,
    },
  });
