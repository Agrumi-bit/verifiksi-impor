...
export async function PATCH(
  ...
  if (visit.locationType !== "PABRIK") {
    return NextResponse.json({ error: "Verifikasi pabrik hanya berlaku untuk lokasi Pabrik." }, { status: 400 });
  }
  ...
  const updated = await db.locationVisit.update({
    where: { id: locationId },
    data: { factoryVerification: parsed.data },
  });

  return NextResponse.json({ data: updated.factoryVerification });
}
