...
import { officeVerificationSchema } from "@/modules/surveyor-workspace/components/office-verification/schema";
...
export async function PATCH(
  ...
  if (visit.locationType !== "KANTOR") {
    return NextResponse.json(
      { error: "Verifikasi kantor hanya berlaku untuk lokasi Kantor." },
      { status: 400 },
    );
  }
  if (visit.status === "COMPLETED") {
    return NextResponse.json(
      { error: "Verifikasi lokasi ini sudah selesai dan tidak dapat diubah." },
      { status: 400 },
    );
  }

  const body = await request.json();
  const parsed = officeVerificationSchema.safeParse(body);
  if (!parsed.success) {
    return NextResponse.json(
  ...
  const updated = await db.locationVisit.update({
    where: { id: locationId },
    data: { officeVerification: parsed.data },
  });

  return NextResponse.json({ data: updated.officeVerification });
}
