...
export function Step4Pajak({ form }: { form: UseFormReturn<CompanyWizardValues> }) {
  ...
  // applies to API-U (Angka Pengenal Impor Umum) — API-P companies never see
  // this question; see the superRefine on companyWizardSchema.
  const apiType = watch("apiType");
  const isApiU = apiType === "API-U";
  ...
  // Stale companyAge from a since-changed API type must never linger — an
  // API-P company that briefly had "API-U" selected in Step 1 shouldn't keep
  // a hidden companyAge value reaching the submit payload.
  useEffect(() => {
    if (!isApiU && companyAge) setValue("companyAge", undefined);
    // eslint-disable-next-line react-hooks/exhaustive-deps -- only react to apiType flipping
  }, [isApiU]);
  ...
  return (
    <div className="flex flex-col gap-5.5">
      ...
      {isApiU && companyAge === "OVER_3" && (
        <CollapsibleCard
          title="Bukti Pembayaran Pajak (3 Tahun Terakhir)"
          description="Opsional — tidak wajib untuk perusahaan yang telah berdiri lebih dari 3 tahun"
          saved={taxSaved}
          open={taxOpen}
            ...
            {taxFields.map((field, index) => {
              ...
              return (
                <div key={field.id} className="rounded-[10px] border border-[#efe2d4] p-3.5">
                  ...
                  <div className="mb-1.5 text-[11.5px] font-semibold text-[#20180f]">Jenis Bukti</div>
                  ...
                  {entry?.type === "ssp" && (
                    <div className="mt-3">
                      <Field label="BPN (Bukti Penerimaan Negara)">
                  ...
                  {entry?.type === "bpe" && (
                    <div className="mt-3">
                      <Field label="NTTE (Nomor Tanda Terima Elektronik)">
