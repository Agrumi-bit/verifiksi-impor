...
import { TECHNICAL_MODULE_STATUS_BADGE, TECHNICAL_MODULE_STATUS_LABELS, type TechnicalModuleStatusValue } from "../status";
    ...
    <div className="flex flex-col overflow-hidden rounded-xl border border-[#f0ded0] bg-white">
      ...
      <div className="flex flex-1 flex-col gap-1.5 p-[18px]">
        ...
        <div className="text-xs text-[#8a7565]">
          {fmtDate(report.validatedAt)} · {report.verificationType}
        </div>
        <div className="mt-1">
          <span className={`rounded-full px-3 py-1 text-[11px] font-bold ${TECHNICAL_MODULE_STATUS_BADGE[report.overallStatus]}`}>
            {TECHNICAL_MODULE_STATUS_LABELS[report.overallStatus]}
          </span>
        </div>
        <Link
          href={`/technical-analyst-workspace/assignments/${report.assignmentNumber}`}
...
export function ReportsTable() {
  ...
  const [status, setStatus] = useState<TechnicalModuleStatusValue | "ALL">("ALL");

  const { data, isLoading, isError } = useQuery({
    queryKey: ["technical-analyst-workspace", "reports"],
    queryFn: async () => {
      const response = await fetch("/api/technical-analyst-workspace/reports");
  ...
  const filtered = useMemo(() => {
    ...
    return (data ?? []).filter((report) => {
      ...
      if (status !== "ALL" && report.overallStatus !== status) return false;
      return true;
    });
  ...
  return (
    <div className="p-7">
      <div className="mb-5">
        ...
        <div className="mt-1 text-[13px] text-[#8a7565]">Laporan analisis teknis yang telah Anda kerjakan.</div>
      </div>
        ...
        {showFilters && (
          <div className="mt-4 grid grid-cols-1 gap-4 border-t border-[#f5ebe1] pt-4 sm:grid-cols-3">
            ...
            <div>
              ...
              <select
                value={status}
                onChange={(event) => setStatus(event.target.value as TechnicalModuleStatusValue | "ALL")}
                className="w-full rounded-lg border-none bg-[#f2f0ee] px-3 py-2.5 text-[13px] text-[#594138] outline-none"
              >
      ...
      {!isLoading && !isError && filtered.length === 0 && (
        <p className="rounded-[10px] border border-[#f0ded0] bg-white p-6 text-center text-[#a68f80]">
          Belum ada laporan analisis yang tersedia.
        </p>
      )}
