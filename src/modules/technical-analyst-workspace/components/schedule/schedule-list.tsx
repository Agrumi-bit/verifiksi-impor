...
function actionMeta(status: ScheduleItem["status"]) {
  ...
  if (scheduleStatus === "ON_PROGRESS") {
    return { label: "Continue Analysis", icon: "edit", className: "bg-[#2f6fe0] text-white" };
  }
  if (scheduleStatus === "SCHEDULED") {
    return { label: "Start Analysis", icon: "play_arrow", className: "bg-[#1a9850] text-white" };
  }
...
export function ScheduleListView({ items }: { items: ScheduleItem[] }) {
  return (
    <div className="overflow-x-auto rounded-[10px] border border-[#f0ded0] bg-white p-6">
      <div className="mb-0.5 text-lg font-extrabold text-[#2b2420]">Daftar Jadwal Analisis</div>
      ...
      <div className="min-w-[900px]">
        ...
        {items.length === 0 && (
          <p className="p-6 text-center text-sm text-[#a68f80]">Belum ada jadwal analisis.</p>
        )}
        ...
        {items.map((item) => {
          ...
          return (
            ...
            >
              ...
              <div>
                <Link
                  href={`/technical-analyst-workspace/assignments/${item.assignmentNumber}`}
