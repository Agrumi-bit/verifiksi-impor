...
const SIDE_NAV_LINKS = [
  { label: "Dashboard", href: "/surveyor-workspace", icon: "dashboard" },
  ...
  return (
    <aside className="fixed left-0 top-0 z-40 hidden h-screen w-[220px] flex-col gap-0.5 border-r border-[#f0ded0] bg-[#fffaf6] py-5 md:flex">
      <div className="mb-3 border-b border-[#f0ded0] px-5 pb-4">
        ...
        <div className="mt-0.5 text-[11px] text-[#a68f80]">Verified Tech #402</div>
      </div>
      <nav className="flex flex-1 flex-col gap-0.5 px-3">
        {SIDE_NAV_LINKS.map((link) => {
          const isActive =
            link.href === "/surveyor-workspace"
              ? pathname === link.href
      ...
      <div className="px-3">
        <button
          type="button"
          onClick={() => toast.info("Sync Offline Data akan tersedia di iterasi berikutnya.")}
