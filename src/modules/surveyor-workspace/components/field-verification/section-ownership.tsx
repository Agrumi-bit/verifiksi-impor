...
import { SurveyorNotes } from "../office-verification/question-list";
  ...
  return (
    <div className="flex gap-2.5">
      <button
        ...
        style={answer.value === "sesuai" ? { background: "#16a34a", color: "#fff" } : { background: "#fff", color: "#4a4038", border: "1px solid #d7dbe0" }}
      >
        Sesuai
      </button>
      <button
        ...
        className="rounded-lg px-4 py-2 text-[13px] font-bold"
        style={answer.value === "tidak" ? { background: "#dc2626", color: "#fff" } : { background: "#fff", color: "#4a4038", border: "1px solid #d7dbe0" }}
      >
        Tidak Sesuai
      </button>
    </div>
...
export function SectionOwnership({
  ...
  const ownershipQ = ownershipQuestion(kind);
  const sewaQs = sewaQuestions(kind);
  ...
  return (
    <SectionShell index={2} title={`Status Kepemilikan ${label}`} onSave={onSave} onSaveNext={onSaveNext} isSaving={isSaving}>
      <p className="mb-3 text-[13.5px] leading-relaxed text-[#4a5568]">
        Verifikasi dilakukan untuk memastikan bahwa status kepemilikan atau penggunaan bangunan {label.toLowerCase()}{" "}
        sesuai dengan dokumen yang disampaikan oleh perusahaan serta kondisi yang ditemukan di lapangan.
      </p>
      <p className="mb-4 text-[13.5px] leading-relaxed text-[#4a5568]">
        Status kepemilikan {label.toLowerCase()} ditentukan berdasarkan data yang disampaikan dalam permohonan.
      </p>
      ...
      <div className="mb-5 flex overflow-hidden rounded-[9px] border border-[#dbe4f0]">
        <div className="flex-1 py-3 text-center text-[13.5px] font-bold" style={buildingStatus === "MILIK_SENDIRI" ? { background: "#3b82f6", color: "#fff" } : { color: "#8a96a8" }}>
          Milik Sendiri
        </div>
        <div className="flex-1 py-3 text-center text-[13.5px] font-bold" style={buildingStatus === "SEWA" ? { background: "#e0662e", color: "#fff" } : { color: "#8a96a8" }}>
          Sewa
        </div>
      </div>
      ...
      {buildingStatus === "MILIK_SENDIRI" && (
        <>
          <div className="mb-5 rounded-xl border border-[#dbe4f0] bg-white p-[18px]">
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <div className="text-[13px] text-[#4a5568]">{ownershipDocPath ? `Dokumen Kepemilikan ${label}` : "Belum diunggah"}</div>
              </div>
              {ownershipDocPath && (
                <button
                  type="button"
          ...
          <div className="mb-5 rounded-xl border border-[#dbe4f0] bg-white p-[18px]">
            <div className="mb-1 text-[14.5px] font-bold text-[#1c2530]">{ownershipQ.title}</div>
            <div className="mb-3 text-[13px] text-[#4a5568]">{ownershipQ.question}</div>
        ...
        <>
          <div className="mb-5 rounded-xl border border-[#dbe4f0] bg-white p-[18px]">
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <div className="text-[13px] text-[#4a5568]">{leaseDocPath ? `Dokumen Sewa Menyewa ${label}` : "Belum diunggah"}</div>
              </div>
              {leaseDocPath && (
                <button
                  type="button"
          ...
          <div className="mb-5 flex flex-col gap-4">
            <div className="rounded-xl border border-[#dbe4f0] bg-white p-[18px]">
              <div className="mb-1 text-[14.5px] font-bold text-[#1c2530]">{sewaQs[0].title}</div>
              <div className="mb-3 text-[13px] text-[#4a5568]">{sewaQs[0].question}</div>
              <AnswerButtons answer={sewaAnswers[sewaQs[0].key] ?? { value: null }} onChange={(v) => onSewaAnswer(sewaQs[0].key, v)} />
            </div>
            ...
            <div className="rounded-xl border border-[#bcd4f5] bg-[#eaf2ff] p-[18px]">
              ...
              <div className="mb-3 text-sm font-bold text-[#1c2530]">{payloadLocation?.leaseOriginalOwnerName || "—"}</div>
            ...
            <div className="rounded-xl border border-[#dbe4f0] bg-white p-[18px]">
              <div className="mb-1 text-[14.5px] font-bold text-[#1c2530]">{sewaQs[1].title}</div>
              <div className="mb-3 text-[13px] text-[#4a5568]">{sewaQs[1].question}</div>
              <AnswerButtons answer={sewaAnswers[sewaQs[1].key] ?? { value: null }} onChange={(v) => onSewaAnswer(sewaQs[1].key, v)} />
              {sewaAnswers[sewaQs[1].key]?.value === "tidak" && (
                <div className="mt-3.5 rounded-[10px] border border-[#f3b8b8] bg-[#fdecec] p-4">
                  ...
                  <textarea
                    value={sewaAnswers[sewaQs[1].key]?.reason ?? ""}
                    onChange={(e) => onSewaAnswer(sewaQs[1].key, { ...sewaAnswers[sewaQs[1].key], reason: e.target.value })}
            ...
            <div className="rounded-xl border border-[#dbe4f0] bg-white p-[18px]">
              <div className="mb-1 text-[14.5px] font-bold text-[#1c2530]">{sewaQs[2].title}</div>
              <div className="mb-3 text-[13px] text-[#4a5568]">{sewaQs[2].question}</div>
              <AnswerButtons answer={sewaAnswers[sewaQs[2].key] ?? { value: null }} onChange={(v) => onSewaAnswer(sewaQs[2].key, v)} />
            </div>
          </div>
        </>
      )}
      ...
      <SurveyorNotes
        ...
        placeholder={`Contoh: ${label} perusahaan menggunakan bangunan yang disewa dengan masa sewa yang masih berlaku hingga saat verifikasi dilakukan.`}
      />

      {previewOpen === "ownership" && ownershipDocPath && (
        <DocumentPreviewModal documentPath={ownershipDocPath} label={`Dokumen Kepemilikan ${label}`} onClose={() => setPreviewOpen(null)} />
      )}
      {previewOpen === "lease" && leaseDocPath && (
        <DocumentPreviewModal documentPath={leaseDocPath} label={`Dokumen Sewa Menyewa ${label}`} onClose={() => setPreviewOpen(null)} />
      )}
    </SectionShell>
  );
}
